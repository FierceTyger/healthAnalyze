...
public class Acquisition {
	...
	public ArrayList<Pair<String, Hashtable<SensorType, ArrayList<Double>>>> retrieveInformationByLogin(
			String loginName,
			ArrayList<Pair<ArrayList<SensorType>, String>> sensorsAndDevices) {
		...
		if (loginName !=null) {
			...
			for (int i = 0; i < sensorsAndDevices.size(); i++) {
				...
				Pair<Integer, Integer> IDsPair = storage.getLoginNameIDs(
						nameMetadataTable, loginName);
				hashAux = storage.retrieveInformationByID(sensors, nameTable,
						IDsPair.first, IDsPair.second);
	...
	/*cc*/
	public ArrayList<Pair<String, Hashtable<SensorType, ArrayList<Double>>>> retrieveInformationByLastID(
			String nameMetaTable,ArrayList<Pair<ArrayList<SensorType>, String>> sensorsAndDevices) {
		ArrayList<Pair<String, Hashtable<SensorType, ArrayList<Double>>>> data = new ArrayList<Pair<String, Hashtable<SensorType, ArrayList<Double>>>>();
		storage.open();
		int MaxId = storage.getMaxIndex(nameMetaTable);
		if (!cm.isStoring())
			storage.close();

		data=retrieveInformationBySession(MaxId,sensorsAndDevices);
